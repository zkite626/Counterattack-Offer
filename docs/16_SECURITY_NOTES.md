...
- 用户自行修改密码和管理员重置密码都必须写入审计日志
- 管理员重置用户密码后必须撤销该用户旧会话
...
- 管理员重置用户密码、重发验证邮件必须写审计日志
- 用户修改邮箱、修改密码必须写审计日志并发送安全提醒或验证邮件
- 管理员不能查看用户 API Key 明文
- 空库首次启动只允许创建默认管理员，并且必须把 `system.bootstrap`
  和 `system.admin.bootstrap` 记录写入审计链路
