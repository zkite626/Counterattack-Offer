...
type ResumeBuilderAction =
  | { type: "LOAD_RESUMES"; payload: ResumeBuilderData[] }
...
function reducer(state: ResumeBuilderState, action: ResumeBuilderAction): ResumeBuilderState {
  switch (action.type) {
    ...
    case "LOAD_RESUMES": {
      const resumes = Object.fromEntries(action.payload.map((resume) => [resume.id, resume]));
      return {
        ...state,
        resumes,
        activeResumeId: state.activeResumeId && resumes[state.activeResumeId]
...
function loadState(): ResumeBuilderState {
  ...
  try {
    ...
    if (saved) {
      const parsed = JSON.parse(saved) as Partial<ResumeBuilderState>;
      return {
        ...INITIAL_STATE,
...
export function ResumeBuilderProvider({ children }: { children: ReactNode }) {
  ...
  }, [state.activeResumeId, state.activeSection]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      resumesApi.update(activeResume)
        .then((saved) => {
  ...
  }, [activeResume, isLoading]);

  const createResume = useCallback(async (templateId: string, title?: string) => {
    const resume = createEmptyResume(templateId, title);
    const saved = await resumesApi.create(resume);
    dispatch({ type: "LOAD_FROM_AI", payload: saved });
    return saved.id;
  }, []);
  ...
  const duplicateResume = useCallback(async (id: string) => {
    const duplicated = await resumesApi.duplicate(id);
    if (duplicated) {
      dispatch({ type: "LOAD_FROM_AI", payload: duplicated });
