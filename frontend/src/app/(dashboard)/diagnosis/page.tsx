...
export default function DiagnosisPage() {
  ...
  const runDiagnosis = useCallback(async () => {
    ...
    try {
      ...
      const normalized = normalizeCareerDiagnosis(result);
  ...
  useEffect(() => {
    if (!diagnosis && state.studentProfile && activeModel) {
      runDiagnosis();
    }
  }, [diagnosis, state.studentProfile, activeModel]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <Card className="biz-page__section">
          <div style={{ textAlign: "center", padding: "var(--space-6) var(--space-4)" }}>
            ...
            {state.studentProfile ? (
              <Button onClick={runDiagnosis} disabled={!activeModel}>
                开始 AI 诊断
              </Button>
            ) : (
