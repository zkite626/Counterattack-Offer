...
export default function JobPage() {
  ...
  async function handleGenerateJd(jobTitle: string) {
    ...
    setError("");
    try {
      const runId = await ensureActiveRun(jobTitle.trim(), state.jobDescription);
      const result = await aiApi.run<{ jd: string }>(
        "generate-jd",
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      const runId = await ensureActiveRun(state.studentProfile?.targetRoles[0] ?? null, jdText);
      ...
      const normalized = normalizeJobAnalysis(result);
