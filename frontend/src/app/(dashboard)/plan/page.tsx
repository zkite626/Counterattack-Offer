...
export default function PlanPage() {
  ...
  const runPlan = useCallback(async () => {
    ...
    try {
      ...
      const normalized = normalizeImprovementPlan(result);
  ...
  }, [state.careerDiagnosis, state.jobAnalysis, state.matchReport, state.jobDescription, activeModel, dispatch, ensureActiveRun]);

  useEffect(() => {
    if (!plan && state.careerDiagnosis && activeModel) {
      runPlan();
    }
  }, [plan, state.careerDiagnosis, activeModel]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <div className="biz-page">
        ...
        <Card className="biz-page__section">
          <div style={{ textAlign: "center", padding: "var(--space-6) var(--space-4)" }}>
            ...
            {missing.length === 0 && (
              <Button onClick={runPlan} disabled={!activeModel}>
                生成行动计划
              </Button>
            )}
          </div>
