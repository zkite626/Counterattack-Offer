...
export default function ResumeBuilderListPage() {
  ...
  const { state, dispatch, createResume, deleteResume, duplicateResume, isLoading } = useResumeBuilder();
  ...
  async function handleCreateBlank() {
    const id = await createResume("classic", "未命名简历");
  ...
  async function handleCreateFromAI() {
    ...
    const saved = await resumesApi.create(resumeData, jobFlowState.activeRunId);
    dispatch({ type: "LOAD_FROM_AI", payload: saved });
    router.push(`/resume-builder/${saved.id}`);
  }
  ...
  return (
    <div className="biz-page resume-builder-list">
      ...
      {isLoading ? (
        <Card className="resume-builder-list__empty">简历加载中...</Card>
      ) : resumeList.length === 0 ? (
