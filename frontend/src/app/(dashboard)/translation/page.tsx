...
export default function TranslationPage() {
  ...
  const runTranslation = useCallback(async () => {
    ...
    try {
      const runId = await ensureActiveRun(state.studentProfile.targetRoles[0] ?? null, state.jobDescription);
      const result = await aiApi.run<ExperienceTranslation[]>(
        "translate",
        {
      ...
      dispatch({ type: "SET_TRANSLATIONS", payload: result });
    } catch (err) {
      setError(err instanceof Error ? err.message : "请求失败");
    } finally {
      setLoading(false);
    }
  }, [state.studentProfile, state.jobDescription, activeModel, dispatch, router, ensureActiveRun]);

  useEffect(() => {
    if (!translations && state.studentProfile && activeModel) {
      runTranslation();
    }
  }, [translations, state.studentProfile, activeModel]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <Card className="biz-page__section">
          <div style={{ textAlign: "center", padding: "var(--space-6) var(--space-4)" }}>
            ...
            {state.studentProfile ? (
              <Button onClick={runTranslation} disabled={!activeModel}>
                开始经历转译
              </Button>
            ) : (
