...
export default function InterviewChat({ jobTitle }: InterviewChatProps) {
  ...
  const { state, ensureActiveRun } = useJobFlow();
  ...
  const sendMessage = useCallback(async () => {
    ...
    if (!text || streamLoading || !activeModel || !state.careerDiagnosis) return;
    ...
    const runId = await ensureActiveRun(jobTitle ?? null, state.jobDescription);

    // 启动后端 AI 代理流，请求体仍只包含业务上下文和模型 ID，不包含 API Key 明文。
    await startStream("/ai/interview", {
      runId,
...
4. 一次只问一个问题`,
  ...
  }, [input, streamLoading, activeModel, messages, jobTitle, startStream, state, ensureActiveRun]);
