...
export default function ResumePage() {
  ...
  useEffect(() => {
    if (!result && state.experienceTranslations && activeModel?.apiKey) {
      runOptimize();
    }
  }, [result, state.experienceTranslations, activeModel?.apiKey]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <Card className="biz-page__section">
          <div style={{ textAlign: "center", padding: "var(--space-6) var(--space-4)" }}>
            ...
            {state.experienceTranslations && (
              <Button onClick={runOptimize} disabled={!activeModel?.apiKey}>
                开始简历优化
              </Button>
            )}
          </div>
